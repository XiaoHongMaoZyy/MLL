...
namespace Mulaolao . Other
{
    public partial class Remind : Form
    {
        ...
        string dataTab ( )
        {
            //存储过程   PEND
            /*
             CREATE PROC [dbo].PEND
AS
BEGIN
IF object_id(N'#LE',N'U') is not null
DROP TABLE #LE
ELSE
CREATE TABLE #LE(
RES01 NVARCHAR(100),
...
INSERT INTO #LE
SELECT DISTINCT A.RES01,B.CX02,C.DBA002,A.RES04,A.RES05,A.RES06,A.RES07,'','','','','' 
FROM R_REVIEWS A INNER JOIN R_MLLCXMC B ON A.RES01=B.CX01 
                 INNER JOIN TPADBA C ON A.RES03=C.DBA001 
                 INNER JOIN R_REVIEW D ON A.RES01=D.RE02 
                 INNER JOIN (SELECT MAX(RES04) RES04 FROM R_REVIEWS  GROUP BY RES01,RES06) E ON A.RES04=E.RES04 
WHERE (SELECT COUNT(1) FROM R_REVIEWS B WHERE A.RES06=B.RES06 AND RES05='执行' )=0  
  AND (SELECT COUNT(1) FROM R_PQW B WHERE A.RES06=B.GZ29 AND GZ39='执行' AND GZ29 IS NOT NULL)=0 
  AND RE01='MLL-0001' ORDER BY RES01,CX02,DBA002,A.RES04,RES05,RES06,RES07

UPDATE #LE SET PQF01=B.PQF01,PQF04=B.PQF04,PQF03=B.PQF03 FROM R_PQL A INNER JOIN R_PQF B ON A.HT01=B.PQF01 WHERE #LE.RES06=A.HT64 collate Chinese_PRC_CI_AI_WS AND RES01='R_021' 
SELECT * FROM #LE
END
             */
            string strWhere = "";
